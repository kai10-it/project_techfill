...
開発初期は、技術的な理解が浅く、ChatGPTを活用してコードの生成や実装を進める場面が多くありました。当時は「まず動くものを作る」ことを優先していたため、処理の仕組みや背景を十分に理解しきれず、コピー&ペーストに近い形で進めていた部分もあります。

しかし、このままでは本質的な成長にはつながらないと感じ、Ruby・Rails・HTML・CSSの基礎をあらためて学習し直しました。その上で、既に作成した各機能を見直し、仕組みを理解しながら一つひとつ丁寧にリファクタリング・再実装を行いました。

このプロセスを経て、単に「動かす」だけでなく、「なぜ動くのか」を理解しながら設計・実装する姿勢を身につけ、自走して開発を進める力が大きく向上したと実感しています。
